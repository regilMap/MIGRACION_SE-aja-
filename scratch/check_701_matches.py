    ...
    for item in grouped_items:
        ...
        for criterion in item["criteria"]:
            ...
            if override_code:
                ...
                best_score = 0.0
                clean_ev = clean_name_for_matching(item['evidence_name'])
            ...
            db_code = best_cand["code"] if best_cand else "NONE"
            db_name = best_cand["name"] if best_cand else "NONE"
            print(f"  Crit: {repr(criterion[:30])} -> Code: {db_code} (Score: {best_score:.2f})")
