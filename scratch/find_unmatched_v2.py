...
for table in extracted_tables:
    ...
    for item in grouped_items:
        ...
        for criterion in criteria_to_process:
            ...
            if sub_code.startswith("5."):
                match = re.search(r'5\.(\d+)', sub_code)
                if match:
                    sub_num = int(match.group(1))
                    if 1 <= sub_num <= 10:
                        crit_lower = criterion.lower()
                        if "no vinculado" in crit_lower or "entre 50" in crit_lower or "50 y" in crit_lower or "50% y" in crit_lower:
...
if unmatched:
    for idx, r in enumerate(unmatched):
        print(f"{idx+1}. Sub: {r['sub_code']} | Ev: {repr(r['pdf_evidence'][:60])} | Crit: {repr(r['pdf_criterion'][:60])}")
