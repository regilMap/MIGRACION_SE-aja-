CREATE OR ALTER PROCEDURE Cache.sp_ActualizarConfiguracionEntidad_Optimizado
...
BEGIN
    ...
    -- Obtener ids de tipos sólo 1 vez
    ...
    BEGIN TRY
        -- 1) CoedomIds: si se pasa @CoedomId usarlo; sino tomar sólo CoedomIds relevantes (evita CROSS JOIN masivo)
        ...
        ELSE
        BEGIN
            -- Tomar sólo CoedomIds que realmente existen en tablas objetivo para reducir cardinalidad
            INSERT INTO #CoedomIds (CoedomId)
            SELECT DISTINCT CODIGO_COEDOM
            FROM dbo.SigerdCoedom
            WHERE CODIGO_COEDOM IS NOT NULL;
            -- Si tienes una relación con una tabla objetivo, filtrar por esa relación aquí (mejor)
        END

        -- 2) Configuraciones generales: obtener la última por (TipoEntidadId, EntidadId)
        ...
        FROM (
            ...
            WHERE IsActive = 1 AND IsDeleted = 0
              -- si se pasa @TipoEntidadNombre, filtrar sólo esos tipos
              AND (
        ...
        -- 3) Excepciones vigentes: tomar la más reciente por (TipoEntidadId, EntidadId, CoedomId)
        ...
        -- Crear tabla staging para cada tipo y convertir lógica compleja en un solo scan
        ----------------------------------------------------------
        -- A) INDICADORES
        IF @TipoEntidadNombre IS NULL OR @TipoEntidadNombre = 'Indicador'
        BEGIN
            -- Generar source temporal para indicadores
            SELECT
            ...
            INNER JOIN #CoedomIds c ON 1 = 1 -- CROSS JOIN reducido: #CoedomIds ya es pequeño; si puedes filtrar por relacion, cámbialo a INNER JOIN con tabla rel.
            ...
            -- Crear índice para acelerar MERGE (clave de matching)
            CREATE CLUSTERED INDEX IX_SrcIndicadores_Key ON #SrcIndicadores (TipoEntidad, EntidadId, CoedomId);

            -- MERGE en Cache.ConfiguracionEntidad desde #SrcIndicadores
