...
for table in extracted_tables:
    ...
    print(f"Grouped items count for 2.03: {len(grouped_items)}")
    for item in grouped_items:
        print(f"Ev: {repr(item['evidence_name'])}")
        print(f"Criteria list ({len(item['criteria'])}):")
        ...
        for criterion in criteria_to_process:
            ...
            print(f"  Matched Criterion: {repr(criterion[:40])} -> Code: {db_code} (Score: {best_score})")
