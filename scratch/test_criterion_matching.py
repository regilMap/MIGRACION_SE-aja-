...
for ev in db_evidences:
    ...
    if match:
        sub_key = f"{int(match.group(1))}.{int(match.group(2)):02d}"  # standardized e.g. "1.01" or "7.01"
...
for page_num in range(18, 76):
    ...
    for t_idx, t in enumerate(tables.tables):
        ...
        # Skip scoring tables and close active table if we hit one
...
# Post-process, group, and match at criterion level
...
for table in extracted_tables:
    ...
    # Get DB candidates for this subindicator
    # E.g. "1.01"
    candidates = db_by_sub.get(sub_code, [])
    if not candidates:
        # Fallback to key without leading zero e.g. "7.01" vs "7.1"
    ...
    for item in grouped_items:
        ...
        # If no criteria, we still output one row with empty criteria
...
matched = [r for r in flat_rows if r["matched_code"]]
...
if unmatched:
    ...
    for r in unmatched[:15]:
        print(f"  Sub: {r['sub_code']} | Ev: {r['pdf_evidence'][:40]}... | Crit: {r['pdf_criterion'][:40]}...")
