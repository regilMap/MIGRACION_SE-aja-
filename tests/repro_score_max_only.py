...
def test_logic():
    ...
    # Case 2: 60, 0, 0 (Previously 60, 30, 0 in greedy. Now MAX ONLY)
    ...
    # Case 4: 51, 0
    assert items[10].Puntuacion == 0.0 # 50 becomes 0 because 51 is max
    assert items[9].Puntuacion == 51.0 # 51 kept (Wait, indices: 10 is ID 400/File10 (50), 11 is ID 400/File11 (51))
    
    # items[9] is ID 11 (51.0)
    # items[8] is ID 10 (50.0) -> Wait, logic: list index:
    # 0, 1, 2 (Case 1)
    # 3, 4, 5 (Case 2)
    # 6, 7 (Case 3)
    # 8, 9 (Case 4) -> items has 10 elements? 0..9.
    
    # Let's recheck indices
    # items[8] -> CargaEvidenciaSource(10, ...)
    # items[9] -> CargaEvidenciaSource(11, ...)
